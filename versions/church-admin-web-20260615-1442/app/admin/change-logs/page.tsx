...
export default async function AdminChangeLogsPage({ searchParams }: PageProps) {
  ...
  const [logs, profiles] = await Promise.all([
    getChangeLogs(params, isAdmin ? null : user.id),
    getProfiles(),
  ]);
  ...
  return (
    <main className="min-h-screen bg-slate-50">
      <Header title={t("admin.logs.title")} locale={profile.language_preference} />
      <section className="mx-auto max-w-7xl space-y-6 px-4 py-6">
        ...
        <form className="grid gap-3 rounded-lg border border-slate-200 bg-white p-4 md:grid-cols-5">
          <label className="grid gap-1 text-sm">
            {t("admin.logs.from")}
            <input className="rounded border border-slate-300 px-3 py-2" name="from" type="date" defaultValue={params.from || ""} />
          </label>
          <label className="grid gap-1 text-sm">
            {t("admin.logs.to")}
            <input className="rounded border border-slate-300 px-3 py-2" name="to" type="date" defaultValue={params.to || ""} />
          </label>
          <label className="grid gap-1 text-sm">
            {t("admin.logs.changedBy")}
            <select className="rounded border border-slate-300 px-3 py-2" name="changed_by" defaultValue={params.changed_by || ""}>
          ...
          <label className="grid gap-1 text-sm">
            {t("admin.logs.tableName")}
            <input className="rounded border border-slate-300 px-3 py-2" name="table_name" defaultValue={params.table_name || ""} />
          </label>
          <label className="grid gap-1 text-sm">
            {t("admin.logs.action")}
            <input className="rounded border border-slate-300 px-3 py-2" name="action" defaultValue={params.action || ""} />
          </label>
        ...
        <SyncedHorizontalScroll contentWidthClassName="min-w-[1000px]">
          <table className="min-w-[1000px] w-full text-sm">
            <thead className="bg-slate-100 text-left">
              <tr>
                <th className="p-3">{t("admin.logs.changedAt")}</th>
                <th className="p-3">{t("admin.logs.changedBy")}</th>
                <th className="p-3">{t("admin.logs.tableName")}</th>
                <th className="p-3">{t("admin.logs.action")}</th>
                <th className="p-3">record_id</th>
                <th className="p-3">{t("admin.logs.details")}</th>
              </tr>
            </thead>
            <tbody>
              {logs.map((log) => (
                <tr key={log.id} className="border-t border-slate-200 align-top">
                  ...
                  <td className="p-3">
                    {log.changed_by ? profileById[log.changed_by]?.full_name || log.changed_by : "-"}
                  </td>
                  ...
                  <td className="p-3">
                    <details>
                      <summary className="cursor-pointer text-sky-700">{t("admin.logs.details")}</summary>
                      <div className="mt-2 grid gap-2 md:grid-cols-2">
                        <pre className="max-h-80 overflow-auto rounded bg-slate-100 p-2 text-xs">
...
async function getChangeLogs(
  ...
  if (params?.from) {
    query = query.gte("changed_at", params.from);
  }

  if (params?.to) {
    query = query.lte("changed_at", `${params.to}T23:59:59`);
  }
  ...
  if (restrictToUserId) {
    query = query.eq("changed_by", restrictToUserId);
  }
