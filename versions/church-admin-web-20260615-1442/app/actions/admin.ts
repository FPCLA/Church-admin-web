...
import { deletedCoworkerNoteMarker, parsePermissionForm } from "@/lib/admin/queries";
...
export async function createCoworker(formData: FormData) {
  ...
  const passwordRedirectTo = `${getAppUrl()}/reset-password`;
...
export async function deleteCoworker(formData: FormData) {
  const { user } = await requireAdminPagePermission("user_management", "manage");
  const profileId = String(formData.get("profile_id") || "");
  const confirmed = formData.get("confirm_delete") === "on";

  if (!profileId || !confirmed) {
    redirect("/admin/users?status=delete_not_confirmed");
  }

  if (profileId === user.id) {
    redirect("/admin/users?status=delete_self_blocked");
  }
  ...
  if (!oldData) {
    redirect("/admin/users?status=user_not_found");
  }

  const now = new Date().toISOString();
  const notes = [
    oldData.notes || "",
    `${deletedCoworkerNoteMarker}${now};deleted_by=${user.id}]`,
  ]
    .filter(Boolean)
    .join("\n");
  ...
  if (profileError) {
    redirect("/admin/users?status=delete_failed");
  }

  await admin.from("user_roles").update({ active: false }).eq("user_id", profileId);
  await admin.from("user_permission_overrides").delete().eq("user_id", profileId);
  await admin.from("record_assignments").delete().eq("user_id", profileId);
