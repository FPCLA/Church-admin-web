...
export default async function DashboardPage({ searchParams }: PageProps) {
  ...
  const modulePermissions = await Promise.all(
    adminModules.map(async (moduleName) => ({
      moduleName,
      canPreview: await hasPermission(user.id, moduleName, "preview"),
      canManage: await hasPermission(user.id, moduleName, "manage"),
    })),
  );
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
              {visibleModules.map((item) => (
                <a
                  ...
                  href={`/modules/${item.moduleName}`}
                  key={item.moduleName}
                >
                  <span className="font-medium">{t(`modules.${item.moduleName}`)}</span>
