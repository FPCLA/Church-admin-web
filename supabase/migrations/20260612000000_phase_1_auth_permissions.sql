...
create trigger set_record_assignments_updated_at
before update on public.record_assignments
for each row execute function public.set_updated_at();
...
using (
  id = auth.uid()
  or public.is_admin()
  or public.has_module_permission('user_management', 'can_manage')
);
...
for select
to authenticated
using (public.is_active_approved());
...
using (
  public.is_admin()
  or public.has_module_permission('permission_management', 'can_manage')
)
with check (
  public.is_admin()
  or public.has_module_permission('permission_management', 'can_manage')
...
create policy user_roles_select_own_or_manage
...
create policy permissions_select_approved
...
create policy overrides_select_own_or_manage
...
create policy record_assignments_select_own_or_manage
...
using (
  user_id = auth.uid()
  or public.is_admin()
  or public.has_module_permission(module_name, 'can_manage')
);

drop policy if exists record_assignments_manage on public.record_assignments;
create policy record_assignments_manage
on public.record_assignments
for all
to authenticated
using (
...
create policy change_logs_select_manage
on public.change_logs
for select
to authenticated
using (
  public.is_admin()
  or public.has_module_permission('change_logs', 'can_preview')
  or public.has_module_permission('change_logs', 'can_manage')
);

drop policy if exists change_logs_insert_self on public.change_logs;
create policy change_logs_insert_self
on public.change_logs
for insert
to authenticated
with check (
...
from public.roles r
on conflict (role_id, module_name) do update
set can_preview = excluded.can_preview;
